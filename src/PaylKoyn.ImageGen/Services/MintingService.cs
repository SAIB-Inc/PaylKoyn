...
)
{
    ...
    public async Task<MintRequest> WaitForPaymentAsync(string id, ulong amount)
    {
        ...
        dbContext.MintRequests.Update(mintRequest);
        await dbContext.SaveChangesAsync();
        throw new TimeoutException($"Payment not received for request ID {id} within the allowed time of {_expirationTime.TotalMinutes} minutes.");
    }

    public async Task<MintRequest> GenerateMetadataAsync(string id)
    {
        using MintDbContext dbContext = dbContextFactory.CreateDbContext();
        MintRequest? mintRequest = await dbContext.MintRequests.FirstOrDefaultAsync(m => m.Id == id)
            ?? throw new ArgumentException($"Mint request with ID {id} not found.");
        ...
        if (mintRequest.Status != MintStatus.PaymentReceived)
            throw new InvalidOperationException($"Mint request with ID {id} is not in payment received status. Current status: {mintRequest.Status}");
        ...
        mintRequest.Status = MintStatus.MetadataGenerated;
        mintRequest.UpdatedAt = DateTime.UtcNow;
        dbContext.MintRequests.Update(mintRequest);
        await dbContext.SaveChangesAsync();

        logger.LogInformation("Metadata generated for request ID: {Id}", id);
        return mintRequest;
    }
    ...
    public async Task<MintRequest> RequestImageUploadAsync(string id)
    {
        ...
        MintRequest? mintRequest = await dbContext.MintRequests.FirstOrDefaultAsync(m => m.Id == id)
            ?? throw new ArgumentException($"Mint request with ID {id} not found.");
        if (mintRequest.Status != MintStatus.MetadataGenerated)
    ...
    public async Task<MintRequest> MintNftAsync(string id, string policyId, string assetName, string asciiAssetName)
    {
        using MintDbContext dbContext = dbContextFactory.CreateDbContext();
        MintRequest? mintRequest = await dbContext.MintRequests.FirstOrDefaultAsync(m => m.Id == id)
            ?? throw new ArgumentException($"Mint request with ID {id} not found.");
        ...
        Metadata cip25Metadata = CreateNftMetadata(
            ...
            asciiAssetName,
            description: "Generated by PaylKoyn"
        );
        ...
        MintNftParams mintNftParams = new(
            ...
            mintRequest.UserAddress, // @TODO: replace with reward address
            cip25Metadata,
